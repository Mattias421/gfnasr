...
def process_audio_and_save_features(
    ...
    try:
        with open(manifest_file, "r", encoding="utf-8") as f:  # Added encoding
            ...
            if manifest_header_lines > 0:
                print(
                    f"Skipping first line of manifest as it seems to be base directory: {lines[0].strip()}"
                )

            for line_num, line in enumerate(
                lines[manifest_header_lines:], start=manifest_header_lines + 1
            ):
                line = line.strip()
                if not line:
                    continue
                ...
                if len(parts) >= 2:
                    try:
                        ...
                        print(
                            f"Warning: Could not parse audio length on line {line_num} of manifest: '{line}'. Skipping."
                        )
                        continue
                else:
                    print(
                        f"Warning: Line {line_num} in manifest does not have at least two tab-separated parts: '{line}'. Skipping."
                    )
    ...
    try:
        with open(transcript_file, "r", encoding="utf-8") as f_trn:  # Added encoding
            raw_transcript_lines = [
                line for line in f_trn.readlines()
            ]  # Keep all lines for now
    except FileNotFoundError:
    ...
    if len(raw_transcript_lines) != num_audio_files:
        print(
            f"Error: Number of audio entries in manifest ({num_audio_files}) "
            f"does not match number of lines in transcript file ({len(raw_transcript_lines)})."
        )
        print(
            "Please ensure the transcript file has one line per valid audio entry from the manifest (after skipping manifest header)."
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            f"Warning: Could not determine embedding dim dynamically: {e}. Using model.dims.n_audio_state if available."
        )
        if hasattr(model.dims, "n_audio_state"):
            ...
            if clean_model_name in dims_map:
                ...
                print(
                    f"Error: Could not determine embedding dimension for model '{whisper_model_name}'."
                )
                sys.exit(1)

    feature_dtype_np = np.float32
    print(
        f"Expected encoder embedding shape per file: ({fixed_encoder_time_dim}, {embedding_dim}), dtype: {feature_dtype_np}"
    )
    ...
    try:
        with h5py.File(output_h5_path, "w") as f_h5:
            ...
            paths_dset = f_h5.create_dataset(
                "relative_filepaths", shape=(num_audio_files,), dtype=dt_str_h5
            )
            lengths_dset = f_h5.create_dataset(
                "wav_lens", shape=(num_audio_files,), dtype=np.int64
            )
            transcripts_dset = f_h5.create_dataset(
                "transcripts", shape=(num_audio_files,), dtype=dt_str_h5
            )  # New dataset
            ...
            for idx, (rel_path, audio_length_samples) in enumerate(audio_entries):
                ...
                print(
                    f"Processing ({idx + 1}/{num_audio_files}): {full_audio_path} (Len: {audio_length_samples} samples)",
                    end=" ",
                )
                ...
                try:
                    ...
                    mel_spectrogram = whisper.log_mel_spectrogram(
                        audio_np_padded, n_mels=model.dims.n_mels, padding=0
                    ).to(model.device)
                    ...
                    if (
                        current_features_np.shape[0] != fixed_encoder_time_dim
                        or current_features_np.shape[1] != embedding_dim
                    ):
                        print(
                        ...
                        embeddings_dset[idx, :, :] = np.zeros(
                            (fixed_encoder_time_dim, embedding_dim),
                            dtype=feature_dtype_np,
                        )
                    else:
                    ...
                    transcripts_dset[idx] = (
                        processed_transcript  # Store processed transcript
                    )
                    ...
                    embeddings_dset[idx, :, :] = np.zeros(
                        (fixed_encoder_time_dim, embedding_dim), dtype=feature_dtype_np
                    )
                    paths_dset[idx] = rel_path  # Store original rel_path
                    lengths_dset[idx] = -1  # Indicate audio error
                    transcripts_dset[idx] = (
                        processed_transcript  # Still store the transcript
                    )
                except Exception as e:
                    print(
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Whisper encoder embeddings, audio lengths, and transcripts; save to HDF5."
    )
    parser.add_argument(
        "manifest_file",
        type=str,
        help="Path to the manifest file (audio paths and lengths).",
    )
    parser.add_argument(
        "transcript_file", type=str, help="Path to the transcript file (.trn)."
    )  # New argument
    parser.add_argument(
        "output_h5_path", type=str, help="Path to save the output HDF5 file."
    )
    parser.add_argument(
        "--base_data_dir",
        type=str,
        ...
        try:
            with open(args.manifest_file, "r", encoding="utf-8") as f:  # Added encoding
                ...
                if os.path.isdir(first_line):
                    ...
                    print(
                        f"Inferred base_data_dir from manifest's first line: {base_data_dir_to_use}"
                    )
                else:
                    print(
                        "Error: --base_data_dir not provided and could not infer from manifest's first line (it's not a directory path)."
                    )
                    sys.exit(1)
        except FileNotFoundError:
