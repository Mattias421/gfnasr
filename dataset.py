...
class HDF5Dataset(Dataset):
    ...
    def __init__(
        ...
        with h5py.File(self.h5_file_path, "r") as f:
            if self.dataset_name not in f:
                raise ValueError(
                    f"Dataset '{self.dataset_name}' not found in HDF5 file '{self.h5_file_path}'. "
            ...
            if self.load_paths_dataset_name:
                if self.load_paths_dataset_name not in f:
                    print(
                        f"Warning: Paths dataset '{self.load_paths_dataset_name}' not found in HDF5 file."
                    )
    ...
    def __getitem__(self, idx):
        ...
        if idx < 0 or idx >= self.num_samples:
            raise IndexError(
                f"Index {idx} out of bounds for dataset with {self.num_samples} samples."
            )
        ...
        with h5py.File(self.h5_file_path, "r") as f:
            ...
            embedding_np = f[self.dataset_name][
                idx
            ]  # This loads the data for the specific index
            ...
            if self.paths_available and self.load_paths_dataset_name:
                try:
                    ...
                    if isinstance(path_info_bytes, bytes):
                        path_info = path_info_bytes.decode("utf-8").strip(
                            "\x00"
                        )  # Decode and strip null chars
        ...
        if path_info is not None:
            ...
            return (
                ...
                wav_len,
                transcript,
            )  # Return embedding and its original path
        else:
